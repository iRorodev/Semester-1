...
void queenmovemnt(int row, int column) // this is how queen moves
{
    ...
    for (int i = 0; i < 8; i++)
    {
        chessboard[row][i] = 'o';    // column wise
        chessboard[i][column] = 'o'; // row wise
    }
    int temprows1 = row - 1;
    int temprows2 = row + 1;
    ...
    while (temprows1 >= 0)
    {
        if (tempcolumns1 >= 0) // top left -->
            chessboard[temprows1][tempcolumns1] = 'o';
        if (tempcolumns2 <= 7) // top right -->
            chessboard[temprows1][tempcolumns2] = 'o';
        tempcolumns1--;
        tempcolumns2++;
        temprows1--;
        temprows2++;
    ...
    while (temprows2 <= 7)
    {
        if (tempcolumns1 >= 0) // bottom left -->
            chessboard[temprows2][tempcolumns1] = 'o';
        if (tempcolumns2 <= 7) // bottom right -->
            chessboard[temprows2][tempcolumns2] = 'o';
    }
...
void knightmovement(int row, int column) // this is how knight moves
{
    ...
    if (row - 2 >= 0 && column - 1 >= 0) // 2 steps up and 1 step left
        chessboard[row - 2][column - 1] = 'o';
    if (row - 2 >= 0 && column + 1 <= 7) // 2 steps up and 1 step right
        chessboard[row - 2][column + 1] = 'o';
    if (row - 1 >= 0 && column - 2 >= 0) // 2 steps left and 1 step up
        chessboard[row - 1][column - 2] = 'o';
    if (row - 1 >= 0 && column + 2 <= 7) // 2 steps right and 1 step up
        chessboard[row - 1][column + 2] = 'o';
    if (row + 1 <= 7 && column - 2 >= 0) // 2 steps left and 1 step down
        chessboard[row + 1][column - 2] = 'o';
    if (row + 1 <= 7 && column + 2 <= 7) // 2 steps right and 1 step down
        chessboard[row + 1][column + 2] = 'o';
    if (row + 2 <= 7 && column - 1 >= 0) // 2 steps down and 1 step left
        chessboard[row + 2][column - 1] = 'o';
    if (row + 2 <= 7 && column + 1 <= 7) // 2 steps down and 1 step right
        chessboard[row + 2][column + 1] = 'o';
}
void rockmovement(int row, int column) // this is how rock moves
{
    ...
    for (int i = 0; i < 8; i++)
    {
        if (chessboard[row][i] != 'R') // to protect the rock from being overwritten
            chessboard[row][i] = 'o';  // row wise
        if (chessboard[i][column] != 'R')
            chessboard[i][column] = 'o'; // column wise
    }
}
void bishopmovement(int row, int column) // this is how bishop moves
{
    cout << "Your piece's location has been marked by \"B\"" << endl;
    cout << "Next Possible movements have been marked by \"o\"" << endl;
    ...
    {
        if (tempcolumns1 >= 0) // top left -->
            chessboard[temprows][tempcolumns1] = 'o';
        if (tempcolumns2 <= 7) // top right -->
            chessboard[temprows][tempcolumns2] = 'o';
        tempcolumns1--;
        tempcolumns2++;
        temprows--;
    }
    temprows = row + 1;
    tempcolumns1 = column - 1;
    tempcolumns2 = column + 1;
    while (temprows <= 7)
    {
        if (tempcolumns1 >= 0) // bottom left -->
            chessboard[temprows][tempcolumns1] = 'o';
...
void putpieces(int &piece) // put pieces in array / chess borard
{
    int row1, row2, col1, col2, kingrow, kingcol;
    srand(time(0));
    kingrow = rand() % 8;
    kingcol = rand() % 8;
...
void findlocationofpieces(int &piece, int &kingrow, int &kingcol, int &piece1row, int &piece1col, int &piece2row, int &piece2col)
// finds the locatuon of pieces since i couldnt figure it how to it in putpieces function.
{
    for (int i = 0; i < 8; i++)
    {
        for (int j = 0; j < 8; j++)
    ...
    if (piece != 1)
    {
        for (int i = piece1row; i < 8; i++)
        {
            for (int j = piece1col + 1; j < 8; j++)
            {
                if (chessboard[i][j] != ' ' && chessboard[i][j] != 'X')
                {
                    piece2row = i;
...
void savetheking() // save the king game
{
    ...
    do
    {
        ...
        {                                                                                    // correct input
            putpieces(piece);                                                                // put the pieces in 8x8 array
            ...
            findlocationofpieces(piece, kingrow, kingcol, piece1row, piece1col, piece2row, piece2col); // this function finds the peiece's location
